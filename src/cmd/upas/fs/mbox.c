...
static Header head[] =
{
	...
	"from:", 		O(x.from),	from822,	0,	1,
	"to:", 		O(x.to),		concat822,	0,	1,
	"sender:",	O(x.sender),	replace822,	0,	1,
	"reply-to:",	O(x.replyto),	replace822,	0,	1,
	"subject:",	O(x.subject),	copy822,		0,	1,
	"cc:", 		O(x.cc),		concat822,	0,	1,
	"bcc:",		O(x.bcc),		concat822,	0,	1,
	"in-reply-to:",	O(x.inreplyto),	replace822,	0,	1,
	"message-id:",	O(x.messageid),	replace822,	0,	1,
...
sanemsg(nm);}
	...
	if(strcmp(rtab[m->x.type].s, "message/rfc822") == 0){
...
parsebody(Message *m, Mailbox *mb)
{
	...
	if(l >= 10 && strncmp(s, "multipart/", 10) == 0)
		...
		/* promote headers */
		if(m->x.replyto == nil && m->x.from == nil && m->x.sender == nil){
			m->x.from = promote(nm->x.from);
			m->x.to = promote(nm->x.to);
			m->date822 = promote(nm->date822);
			m->x.sender = promote(nm->x.sender);
			m->x.replyto = promote(nm->x.replyto);
			m->x.subject = promote(nm->x.subject);
		}
	}else if(strncmp(rtab[m->x.type].s, "text/", 5) == 0)
		sanemsg(m);
	m->x.rawbsize = m->rbend - m->rbody;
	m->cstate |= Cbody;
}
...
from822(Message *m, Header *h, char*, char *p)
{
	if(m->x.ffrom)
		free(m->x.ffrom);
	m->x.from = 0;
	return addr822(p + h->len, &m->x.ffrom);
}
...
cdisposition(Message *m, Header *h, char*, char *p)
{
	for(p = skipwhite(p + h->len); *p; p = skiptosemi(p))
		if(cistrncmp(p, "inline", 6) == 0)
			m->x.disposition = Dinline;
		else if(cistrncmp(p, "attachment", 10) == 0)
			m->x.disposition = Dfile;
...
newmessage(Message *parent)
{
	...
	m->x.disposition = Dnone;
	m->x.type = newrefs("text/plain");
...
decode(Message *m)
{
	...
	case Ebase64:
		...
		if(strncmp(rtab[m->x.type].s, "text/", 5) == 0)
			len = deccr(x, len);
		if(m->ballocd)
			free(m->body);
		m->body = x;
...
convert(Message *m)
{
	...
	if(m->converted)
		return;
	m->converted = 1;
	if(m->part != nil || cistrncmp(rtab[m->x.type].s, "text", 4) != 0)
		return;
...
mailplumb(Mailbox *mb, Message *m, int delete)
{
	...
	snprint(date, sizeof date, "%Δ", m->x.fileid);
	a[++ai].name = "date";
	a[ai].value = date;
	a[ai-1].next = &a[ai];

	if(m->x.digest){
		snprint(dbuf, sizeof dbuf, "%A", m->x.digest);
		a[++ai].name = "digest";
		a[ai].value = dbuf;
		a[ai-1].next = &a[ai];
	}
