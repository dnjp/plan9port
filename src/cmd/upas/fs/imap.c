	...
	for(i = 0; i < n; i += 2){
		if(strcmp(f[i], "internaldate") == 0){
			...
			if(o == -1)
				return confused;
			l = xnum(f[i + 1], '{', '}');
			a = o + l - m->x.ibadchars - m->x.size;
			if(a > 0){
				assert(imap->flags & Fgmail);
				m->x.size = o + l;
				msgrealloc(m, m->x.size);
				m->x.size -= m->x.ibadchars;
			}
...
imap4read(Imap *imap, Mailbox *mb, int doplumb, int *new)
{
	...
	*new = 0;
	...
	qsort(f, n, sizeof f[0], (int (*)(const void*, const void*))fetchicmp);
	nnew = ndel = 0;
	ll = &mb->root->part;
	for(i = 0; *ll || i < n; ){
		c = -1;
		if(i >= n)
			...
			if((*ll)->imapuid == 0)
				(*ll)->imapuid = strtoull((*ll)->x.idxaux, 0, 0);
		...
		if(c < 0){
			...
			m->inmbox = 1;
			m->x.idxaux = smprint("%llud", f[i].uid);
			m->imapuid = f[i].uid;
			m->x.fileid = datesec(imap, i);
			if(setsize(mb, m, f + i) < 0 || m->x.size >= Maxmsg){
