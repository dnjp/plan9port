...
mdirfetch(Mailbox *mb, Message *m, uvlong o, ulong l)
{
	...
	mdir = mb->aux;
	mdprint(mdir, "mdirfetch(%D) ...", m->x.fileid);

	snprint(buf, sizeof buf, "%s/%D", mb->path, m->x.fileid);
...
mdirread(Mdir* mdir, Mailbox* mb, int doplumb, int *new)
{
	...
	*new = nnew = 0;
	...
	qsort(d, n, sizeof *d, (int (*)(const void*, const void*))dircmp);
	ndel = 0;
	ll = &mb->root->part;
	for(i = 0; *ll || i < n; ){
		...
		if(i >= n)
			...
			c = vcmp(uv, (*ll)->x.fileid);
		mdprint(mdir, "consider %s and %D -> %d\n", i<n? d[i].name: 0, *ll? (*ll)->x.fileid: 1ull, c);
		if(c < 0){
			/* new message */
			mdprint(mdir, "new: %s (%D)\n", d[i].name, *ll? (*ll)->x.fileid: 0);
			m = newmessage(mb->root);
			m->x.fileid = uv;
			if(setsize(mb, m) < 0 || m->x.size >= Maxmsg){
			...
			*ll = m;
			...
			/* deleted message; */
			mdprint(mdir, "deleted: %s (%D)\n", i<n? d[i].name: 0, *ll? (*ll)->x.fileid: 0);
			ndel++;
...
mdirdelete(Mailbox *mb, Message *m)
{
	...
	mdir = mb->aux;
	snprint(mpath, sizeof mpath, "%s/%D", mb->path, m->x.fileid);
