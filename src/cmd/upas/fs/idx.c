...
pridxmsg(Biobuf *b, Idx *x)
{
	...
	Bprint(b, "%q %q %q %q %q ", es(x->ffrom), es(x->from), es(x->to), es(x->cc), es(x->bcc));
	Bprint(b, "%q %q %q %q %q ", es(x->replyto), es(x->messageid), es(x->subject), es(x->sender), es(x->inreplyto));
	Bprint(b, "%s %d %lud %lud ", rtab[x->type].s, x->disposition, x->size, x->rawbsize);
	Bprint(b, "%lud %q %d\n", x->ibadchars, es(x->idxaux), x->nparts);
	return 0;
}
...
validmessage(Mailbox *mb, Message *m, int level)
{
	if(level){
		...
		if(m->x.fileid <= 1000000ull<<8)
			goto lose;
		if(mtreefind(mb, m->x.digest))
			goto lose;
	}
	return 1;
lose:
	eprint("invalid cache[%d] %#A size %ld %D\n", level, m->x.digest, m->x.size, m->x.fileid);
	return 0;
}
...
dead:
		...
		if(m){
			...
			if(level == 0)
				m->deleted &= ~Dmark;
			if(m->x.nparts)
			if(rdidx(b, mb, m, m->x.nparts, level + 1, 0) == -1)
				goto dead;
			ll = &m->next;
			idprint("%d seen before %d... %.2ux", level, m->id, m->cstate);
			flags = m->x.flags;
			m->x.flags |= strtoul(f[1], 0, 16);
			if(flags != m->x.flags)
				m->cstate |= Cidxstale;
			m->cstate |= Cidx;
			idprint("→%.2ux\n", m->cstate);
		...
		m->x.bcc = ns(f[8]);
		m->x.replyto = ns(f[9]);
		m->x.messageid = ns(f[10]);
		m->x.subject = ns(f[11]);
		m->x.sender = ns(f[12]);
		m->x.inreplyto = ns(f[13]);
		m->x.type = newrefs(f[14]);
		m->x.disposition = atoi(f[15]);
		m->x.size = strtoul(f[16], 0, 0);
		m->x.rawbsize = strtoul(f[17], 0, 0);
		m->x.ibadchars = strtoul(f[18], 0, 0);
		m->x.idxaux = ns(f[19]);
		m->x.nparts = strtoul(f[20], 0, 0);
		m->cstate &= ~Cidxstale;
		m->cstate |= Cidx;
		m->x.str = s;
		s = 0;
